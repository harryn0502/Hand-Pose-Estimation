...
for img_path in img_path_list:
    ...
    for hand_type in ('right', 'left'):
        ...
        face = torch.from_numpy(mano_layer[hand_type].faces.astype(np.int32)).cpu()[None,:,:]
        focal = torch.FloatTensor(cam_param['focal'][str(cam_idx)]).cpu()[None,:]
        princpt = torch.FloatTensor(cam_param['princpt'][str(cam_idx)]).cpu()[None,:]
        with torch.no_grad():
