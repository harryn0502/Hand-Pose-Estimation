...
class PoseNet(nn.Module):
    ...
    def soft_argmax_1d(self, heatmap1d, device):
        heatmap1d = F.softmax(heatmap1d, 1)
        #accu = heatmap1d * torch.arange(cfg.output_root_hm_shape).float().cuda()[None,:]
        accu = heatmap1d * torch.arange(cfg.output_root_hm_shape).float().to(device)[None,:]
    ...
    def forward(self, img_feat):
        ...
        root_depth = self.soft_argmax_1d(root_heatmap1d, device).view(-1,1)
